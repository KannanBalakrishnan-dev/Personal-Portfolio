import {
  require_jsx_runtime
} from "./chunk-SICDWW74.js";
import {
  require_react
} from "./chunk-BVJBJJA6.js";
import {
  __require,
  __toESM
...
function createArcPath({ strength = 0.5, peak = 0.5, direction, rotate: rotate2 = false } = {}) {
  const rotationScale = rotate2 === true ? 1 : typeof rotate2 === "number" ? rotate2 : 0;
  let prevBulgeSign;
  ...
  const path = {
    ...
    animateVisualElement(visualElement, target, transition, delay2, animations2) {
      ...
      progress2.start(animateMotionValue("", progress2, [0, 1e3], {
        ...
        velocity: 0,
        onUpdate: (latest) => {
          const point = interpolate2(latest / 1e3);
          xValue?.set(point.x);
          yValue?.set(point.y);
          if (pathRotationValue && point.rotate !== void 0) {
...
function startViewAnimation(builder) {
  ...
  const resolveLayerTransition = (target, type, transitionName, index, total) => {
    const transition2 = mergeTransition(getValueTransition(defaultOptions, transitionName), getValueTransition(layerOptions(target, type) ?? {}, transitionName));
  ...
  const resolveGroupTiming = (name) => {
    ...
    const { delay: delay2 = 0, duration, ease: ease2 } = applyGeneratorOptions(transition2);
    return { delay: secondsToMilliseconds(delay2), duration, ease: ease2 };
  };
    ...
    transition.ready.then(() => {
      ...
      layerTargets.forEach((target, name) => {
        ...
        for (const key of definitionNames) {
          ...
          if (index === -1)
            continue;
          const { keyframes: keyframes2, options } = target[key];
      ...
      for (const animation of generatedViewAnimations) {
        ...
        if (name.type.startsWith("group")) {
          const { delay: delay2, duration, ease: ease2 } = resolveGroupTiming(name.layer);
          timing = {
            delay: delay2,
            duration,
      ...
      cropMeasurements.forEach((entry, name) => {
        if (!croppedNames.has(name))
          return;
        const { delay: delay2, duration, ease: ease2 } = resolveGroupTiming(name);
        for (const corner of cornerRadiusProps) {
...
var LayoutAnimationBuilder = class {
  ...
  shared(id3, transition) {
    this.sharedTransitions.set(id3, transition);
    return this;
  }
  then(resolve, reject) {
    return this.readyPromise.then(resolve, reject);
...
var makeUseVisualState = (config) => (props, isStatic) => {
  const context = (0, import_react6.useContext)(MotionContext);
...
var SwitchLayoutGroupContext = (0, import_react7.createContext)({});
...
function useIsPresent() {
  return isPresent((0, import_react9.useContext)(PresenceContext));
}
...
var createDomVisualElement = (Component2, options) => {
  ...
  return isSVG ? new SVGVisualElement(options) : new HTMLVisualElement(options, {
    allowProjection: Component2 !== import_react11.Fragment
  });
};
  ...
  snapToCursor(point) {
    eachAxis((axis) => {
      ...
      if (projection && projection.layout) {
        ...
        axisValue.set(point[axis] - mixNumber(min, max, 0.5) + current2);
      }
    });
  }
  /**
...
function MeasureLayout(props) {
  ...
  return (0, import_jsx_runtime.jsx)(MeasureLayoutWithContext, { ...props, layoutGroup, switchLayoutGroup: (0, import_react12.useContext)(SwitchLayoutGroupContext), isPresent: isPresent2, safeToRemove });
}
...
function useVisualElement(Component2, visualState, props, createVisualElement2, ProjectionNodeConstructor, isSVG) {
  ...
  const motionConfig = (0, import_react18.useContext)(MotionConfigContext);
  const reducedMotionConfig = motionConfig.reducedMotion;
  const skipAnimations = motionConfig.skipAnimations;
  const visualElementRef = (0, import_react18.useRef)(null);
  const hasMountedOnce = (0, import_react18.useRef)(false);
  ...
  const wantsHandoff = (0, import_react18.useRef)(Boolean(optimisedAppearId) && typeof window !== "undefined" && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
...
// node_modules/framer-motion/dist/es/motion/index.mjs
function createMotionComponent(Component2, { forwardMotionProps = false, type } = {}, preloadedFeatures, createVisualElement2) {
  ...
  function MotionDOMComponent(props, externalRef) {
    ...
    if (!isStatic && typeof window !== "undefined") {
      ...
      context.visualElement = useVisualElement(Component2, visualState, configAndProps, createVisualElement2, layoutProjection.ProjectionNode, isSVG);
    }
  ...
  const ForwardRefMotionComponent = (0, import_react19.forwardRef)(MotionDOMComponent);
...
//# sourceMappingURL=chunk-ZKOWKBY3.js.map
